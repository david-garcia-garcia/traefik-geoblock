...
Entrypoints stay at `github.com/david-garcia-garcia/traefik-geoblock`. Helpers live in `pkg/<name>`. Root `New` reuses one Plugin; `pkg/geoblock` constructs, binds wrappers, and serves.

## How to use

- Root package exports `Config`, `CreateConfig`, and `New`. Those are the Yaegi entrypoints. Root `New` calls `Prepare` (so the instance key is the normalized config), `reclaim.Open`s the incarnation, and `ForRoute`s this `next`.
- Put Plugin, ServeHTTP, Prepare, NewCore, and ForRoute in `pkg/geoblock`. ForRoute opens the DatabaseProvider with this `New` context. That package does not call `reclaim.Open`.
...
- `pkg/geoblock` — Plugin, ServeHTTP, Prepare, NewCore, ForRoute
