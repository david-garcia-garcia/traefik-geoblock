...
- Third-party packages (reclaim, iplookup, ip2location, oschwald) live in `vendor/`. Do not copy them into `pkg/`.
...
- `pkg/` — helpers (`dbprovider`, `dbwrappers`, `dbsource`, `dbutils`, `fileutils`, `logging`)
...
- `vendor/github.com/david-garcia-garcia/traefik-middleware-utilities/iplookup` — family-isolated CIDR helper (third-party; do not fork into `pkg/`). `pkg/geoblock` loads static + directory lists onto it.
