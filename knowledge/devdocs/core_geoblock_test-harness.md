...
- Change Traefik-visible behavior (headers, labels, blocking) → add a `whoami-*` service in `docker-compose.yml` with a unique `PathPrefix` and matching plugin labels, then a Pester `Context` / `It` in `scripts/integration-tests.Tests.ps1`. Shared-middleware incarnation + config-change grace is `/reclaima` `/reclaimb` `/reclaimc` and the Pester Context `Shared middleware incarnation and config change`. Enrich then block on one router is `/enrichthenblock` (`geoblock-chain-enrich` then `geoblock-chain-block`). Block with no prior lookup is `/blockonly`.
