...
`NewCore` opens enabled `databaseSources` only when `mode` is `enrich` or `enrichandblock`. Request path calls `Lookup` and gets a `Record`, then writes `countryHeader` and `requestHeaderEnrich`. An empty field is the string `null`. Country on a private IP is `PRIVATE`. `block` and `disabled` do not open catalog sources. A `bin` LITE DB1 is country-only. Region/city/ISP/domain need a richer BIN. ASN LITE is a second `bin` row with `fieldsPreconfigured: ip2location_asn` (`Get_asn`; no shipped seed). An `mmdb` row walks the operator or preset path map on a decoded map. The bundled seeds are `ipinfo_lite.mmdb` and official dummy `GeoIP2-Country-Test.mmdb`. Shipped `default_geolite` is a disabled unofficial Country GET.

## How to use

- Enable catalog rows (`databaseType`, optional `defaultFile`, `fields` or `fieldsPreconfigured`). `fields` is on-disk path → Record key. Do not combine the two. Omitted `enabled` is on. Disable `default_ip2location` when another country source should be the only one.
