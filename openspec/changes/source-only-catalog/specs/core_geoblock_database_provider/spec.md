...
Vendor Lookup SHALL live in that vendor’s package. Open and hot-swap SHALL live in `pkg/dbwrappers`. HTTP GET and Resolve SHALL stay in `pkg/dbsource`. A later vendor MUST be addable by implementing Lookup for a new `vendor` value and a catalog row. The plugin MUST NOT type-assert a format wrapper to read source state.
