...
- [x] 2.1 Open one wrapper per enabled row (`ip2location` / `ip2location-asn` BIN, `ipinfo` / `maxmind` MMDB). Field maps on `BINSource` / `ASNSource` / `IPinfo` / `GeoIP2`. Remove `pkg/ip2location`, `pkg/ipinfo`, `pkg/maxmind`.
