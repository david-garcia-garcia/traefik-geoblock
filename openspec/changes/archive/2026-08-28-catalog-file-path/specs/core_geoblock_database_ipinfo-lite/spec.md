...
IPinfo-only settings SHALL live on Traefik Config as `ipinfo_download`. The plugin MUST pass that field to the IPinfo provider only. File location SHALL be the catalog `path` and dated files as specified in `core_geoblock_database_url-download`. IPinfo MUST NOT use IP2Location `file=` query codes or a vendor-built `ipinfo_{code}.mmdb?token=` URL. Config SHALL NOT expose `ipinfo_databaseFilePath`.

#### Scenario: Pointer reaches IPinfo only
- **WHEN** `databaseProvider` is `ipinfo` and `ipinfo_download` names a catalog entry whose `path` is an existing MMDB
...
- **WHEN** `databaseProvider` is `ipinfo` and `ipinfo_download` is empty and no dated catalog file exists
